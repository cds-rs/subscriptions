## Resume rejected at the cancelled period end — PASS
...
### The merchant's pull is refused: the cancelled period has ended

**TransferSubscription (after cancelled period): structured CPI tree**

```text

Transaction  signers=[merchant]
└── subscriptions::TransferSubscription [1] ✗ 1116cu  signer=merchant
    └── Error: SubscriptionCancelled
Error: InstructionError(0, Custom(508))
Compute Units (this run): 1116
Fee: 5000 lamports
Legend (2):
  merchant      = J4fPsxKiTTKiXSiN9bgZ5JBrVgTM9c6N8tjhLN8gfdTq
  subscriptions = De1egAFMkMWZSN5rYXRj9CAdheBamobVNubTsi9avR44
...
### Alice's resume is refused for the same reason

**ResumeSubscription (after cancelled period): structured CPI tree**

```text

Transaction  signers=[alice]
└── subscriptions::ResumeSubscription [1] ✗ 462cu  signer=alice
    └── Error: SubscriptionCancelled
Error: InstructionError(0, Custom(508))
Compute Units (this run): 462
Fee: 5000 lamports
Legend (2):
  alice         = FXddRd8CdAC8SWKT3Ataasn69R7rbTfQZcKg8ejyrUbF
  subscriptions = De1egAFMkMWZSN5rYXRj9CAdheBamobVNubTsi9avR44
