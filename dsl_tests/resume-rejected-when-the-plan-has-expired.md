## Resume rejected when the plan has expired — PASS
...
flowchart LR
    ...
    7kefTio6_vztQ[("7kefTio6…vztQ")]:::writable
    ...
    subscriptions -->|writes| 7kefTio6_vztQ
    Token -->|writes| 7kefTio6_vztQ
```
...
flowchart LR
    ...
    Token[Token]:::owner
    7kefTio6_vztQ[("7kefTio6…vztQ")]:::account
    System -->|owns| alice
    subscriptions -->|owns| SubAuthority
    Token -->|owns| 7kefTio6_vztQ
```
...
### Alice resumes after the plan has expired

**ResumeSubscription (plan expired): structured CPI tree**

```text

── subscriptions::ResumeSubscription ───────────────────────
Transaction  signers=[alice]
└── subscriptions::ResumeSubscription [1] ✗ 457cu  signer=alice
    └── Error: PlanExpired (0x1f5)
Error: InstructionError(0, Custom(501))
Compute Units (this run): 457
Fee: 5000 lamports
Legend (2):
  subscriptions = De1egAFMkMWZSN5rYXRj9CAdheBamobVNubTsi9avR44
  alice         = FXddRd8CdAC8SWKT3Ataasn69R7rbTfQZcKg8ejyrUbF
