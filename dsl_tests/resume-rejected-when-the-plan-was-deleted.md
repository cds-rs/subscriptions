## Resume rejected when the plan was deleted — PASS

> resuming against a closed (deleted) plan is refused

### Stage: Alice's authority, the merchant's tight 1-hour plan, Alice subscribed
...
flowchart LR
    ...
    9BResw4S_Wmb7[("9BResw4S…Wmb7")]:::writable
    ...
    subscriptions -->|writes| 9BResw4S_Wmb7
    Token -->|writes| 9BResw4S_Wmb7
```
...
flowchart LR
    ...
    Token[Token]:::owner
    9BResw4S_Wmb7[("9BResw4S…Wmb7")]:::account
    System -->|owns| alice
    subscriptions -->|owns| SubAuthority
    Token -->|owns| 9BResw4S_Wmb7
```
...
### Alice resumes against the deleted plan

**ResumeSubscription (plan closed): structured CPI tree**

```text

── subscriptions::ResumeSubscription ───────────────────────
Transaction  signers=[alice]
└── subscriptions::ResumeSubscription [1] ✗ 409cu  signer=alice
    └── Error: PlanClosed (0x204)
Error: InstructionError(0, Custom(516))
Compute Units (this run): 409
Fee: 5000 lamports
Legend (2):
  subscriptions = De1egAFMkMWZSN5rYXRj9CAdheBamobVNubTsi9avR44
  alice         = FXddRd8CdAC8SWKT3Ataasn69R7rbTfQZcKg8ejyrUbF
