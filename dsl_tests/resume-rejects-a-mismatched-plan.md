## Resume rejects a mismatched plan — PASS
...
flowchart LR
    ...
    8wCLXXsZ_o2D5[("8wCLXXsZ…o2D5")]:::writable
    ...
    subscriptions -->|writes| 8wCLXXsZ_o2D5
    Token -->|writes| 8wCLXXsZ_o2D5
```
...
flowchart LR
    ...
    Token[Token]:::owner
    8wCLXXsZ_o2D5[("8wCLXXsZ…o2D5")]:::account
    System -->|owns| alice
    subscriptions -->|owns| SubAuthority
    Token -->|owns| 8wCLXXsZ_o2D5
```
...
**CreatePlan (second plan): structured CPI tree**
...
**CreatePlan (second plan): sequence diagram**
...
**CreatePlan (second plan): sequence diagram, with lifelines**
...
**CreatePlan (second plan): authority graph**
...
flowchart LR
    ...
    subscriptions[subscriptions]:::program
    merchant([merchant]):::signer
    Wrong_Plan(["Wrong Plan"]):::signer
    System[System]:::program
    merchant -->|signs| subscriptions
    merchant -->|signs| System
    Wrong_Plan -->|signs| System
    subscriptions -->|writes| Wrong_Plan
```

**CreatePlan (second plan): ownership graph**
...
flowchart LR
    ...
    merchant[(merchant)]:::account
    subscriptions[subscriptions]:::owner
    Wrong_Plan[("Wrong Plan")]:::account
    System -->|owns| merchant
    subscriptions -->|owns| Wrong_Plan
```

### Alice resumes against the wrong plan

**ResumeSubscription (plan mismatch): structured CPI tree**

```text

── subscriptions::ResumeSubscription ───────────────────────
Transaction  signers=[alice]
└── subscriptions::ResumeSubscription [1] ✗ 390cu  signer=alice
    └── Error: SubscriptionPlanMismatch (0x1f9)
Error: InstructionError(0, Custom(505))
Compute Units (this run): 390
Fee: 5000 lamports
Legend (2):
  subscriptions = De1egAFMkMWZSN5rYXRj9CAdheBamobVNubTsi9avR44
  alice         = FXddRd8CdAC8SWKT3Ataasn69R7rbTfQZcKg8ejyrUbF
