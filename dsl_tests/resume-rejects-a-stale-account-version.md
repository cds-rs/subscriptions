## Resume rejects a stale account version — PASS
...
flowchart LR
    ...
    8bKKBE8s_oMBr[("8bKKBE8s…oMBr")]:::writable
    ...
    subscriptions -->|writes| 8bKKBE8s_oMBr
    Token -->|writes| 8bKKBE8s_oMBr
```
...
flowchart LR
    ...
    Token[Token]:::owner
    8bKKBE8s_oMBr[("8bKKBE8s…oMBr")]:::account
    System -->|owns| alice
    subscriptions -->|owns| SubAuthority
    Token -->|owns| 8bKKBE8s_oMBr
```
...
### Alice resumes; the stale version is refused

**ResumeSubscription (version mismatch): structured CPI tree**

```text

── subscriptions::ResumeSubscription ───────────────────────
Transaction  signers=[alice]
└── subscriptions::ResumeSubscription [1] ✗ 358cu  signer=alice
    └── Error: MigrationRequired (0x86)
Error: InstructionError(0, Custom(134))
Compute Units (this run): 358
Fee: 5000 lamports
Legend (2):
  subscriptions = De1egAFMkMWZSN5rYXRj9CAdheBamobVNubTsi9avR44
  alice         = FXddRd8CdAC8SWKT3Ataasn69R7rbTfQZcKg8ejyrUbF
