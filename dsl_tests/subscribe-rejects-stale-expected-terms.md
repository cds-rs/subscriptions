## Subscribe rejects stale expected terms — PASS
...
### Alice subscribes carrying a stale expected_amount

**Subscribe (stale terms): structured CPI tree**

```text

Transaction  signers=[alice]
└── subscriptions::Subscribe [1] ✗ 2163cu  signer=alice
    └── Error: PlanTermsMismatch
Error: InstructionError(0, Custom(519))
Compute Units (this run): 2163
Fee: 5000 lamports
Legend (2):
  alice         = FXddRd8CdAC8SWKT3Ataasn69R7rbTfQZcKg8ejyrUbF
  subscriptions = De1egAFMkMWZSN5rYXRj9CAdheBamobVNubTsi9avR44
