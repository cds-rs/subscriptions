## Transfer subscription: a cancelled subscription past its period is refused — PASS
...
flowchart LR
    ...
    22mMrcos_QEWn[("22mMrcos…QEWn")]:::writable
    ...
    subscriptions -->|writes| 22mMrcos_QEWn
    Token -->|writes| 22mMrcos_QEWn
```
...
flowchart LR
    ...
    Token[Token]:::owner
    22mMrcos_QEWn[("22mMrcos…QEWn")]:::account
    System -->|owns| alice
    subscriptions -->|owns| SubAuthority
    Token -->|owns| 22mMrcos_QEWn
```
...
### The merchant pulls against a subscription past its cancellation period

**TransferSubscription (subscription cancelled): structured CPI tree**

```text

── subscriptions::TransferSubscription ─────────────────────
Transaction  signers=[merchant]
└── subscriptions::TransferSubscription [1] ✗ 1122cu  signer=merchant
    └── Error: SubscriptionCancelled (0x1fc)
Error: InstructionError(0, Custom(508))
Compute Units (this run): 1122
Fee: 5000 lamports
Legend (2):
  subscriptions = De1egAFMkMWZSN5rYXRj9CAdheBamobVNubTsi9avR44
  merchant      = J4fPsxKiTTKiXSiN9bgZ5JBrVgTM9c6N8tjhLN8gfdTq
