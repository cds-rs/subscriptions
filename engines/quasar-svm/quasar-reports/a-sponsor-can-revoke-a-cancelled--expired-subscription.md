## A sponsor can revoke a cancelled, expired subscription — PASS
...
flowchart LR
    ...
    DCtqPvzM_jtus[("DCtqPvzM…jtus")]:::writable
    ...
    subscriptions -->|writes| DCtqPvzM_jtus
    Token -->|writes| DCtqPvzM_jtus
```
...
flowchart LR
    ...
    Token[Token]:::owner
    DCtqPvzM_jtus[("DCtqPvzM…jtus")]:::account
    System -->|owns| alice
    subscriptions -->|owns| SubAuthority
    Token -->|owns| DCtqPvzM_jtus
```
...
### The sponsor revokes the cancelled subscription

**RevokeSubscription (cancelled and expired): structured CPI tree**

```text

── subscriptions::RevokeDelegation ─────────────────────────
Transaction  signers=[sponsor]
└── subscriptions::RevokeDelegation [1] ✓ 532cu  signer=sponsor
...
**RevokeSubscription (cancelled and expired): sequence diagram**
...
sequenceDiagram
    ...
    participant sponsor
    participant subscriptions
    sponsor ->> subscriptions: RevokeDelegation (532cu)
```

**RevokeSubscription (cancelled and expired): sequence diagram, with lifelines**
...
sequenceDiagram
    ...
    subscriptions -->>- sponsor: ok (532cu)
```

**RevokeSubscription (cancelled and expired): authority graph**
...
**RevokeSubscription (cancelled and expired): ownership graph**
