## A subscriber revoke routes rent to the sponsor — PASS
...
flowchart LR
    ...
    DbfBm2dr_7afp[("DbfBm2dr…7afp")]:::writable
    ...
    subscriptions -->|writes| DbfBm2dr_7afp
    Token -->|writes| DbfBm2dr_7afp
```
...
flowchart LR
    ...
    Token[Token]:::owner
    DbfBm2dr_7afp[("DbfBm2dr…7afp")]:::account
    System -->|owns| alice
    subscriptions -->|owns| SubAuthority
    Token -->|owns| DbfBm2dr_7afp
```
...
### Alice revokes, routing the rent back to the sponsor

**RevokeSubscription (rent to sponsor): structured CPI tree**

```text

── subscriptions::RevokeDelegation ─────────────────────────
Transaction  signers=[alice]
└── subscriptions::RevokeDelegation [1] ✓ 480cu  signer=alice
Compute Units (this run): 480
Fee: 0 lamports
Legend (2):
  subscriptions = De1egAFMkMWZSN5rYXRj9CAdheBamobVNubTsi9avR44
  alice         = FXddRd8CdAC8SWKT3Ataasn69R7rbTfQZcKg8ejyrUbF
...
**RevokeSubscription (rent to sponsor): sequence diagram**
...
sequenceDiagram
    ...
    participant alice
    participant subscriptions
    alice ->> subscriptions: RevokeDelegation (480cu)
```

**RevokeSubscription (rent to sponsor): sequence diagram, with lifelines**
...
sequenceDiagram
    ...
    subscriptions -->>- alice: ok (480cu)
```

**RevokeSubscription (rent to sponsor): authority graph**
...
**RevokeSubscription (rent to sponsor): ownership graph**
