...
Transaction  signers=[alice]
└── subscriptions::CreateRecurringDelegation [1] ✓ 11073cu  signer=alice
    └── System::CreateAccount [2] ✓ (no cu)
Compute Units (this run): 11073
Fee: 0 lamports
Legend (2):
  subscriptions = De1egAFMkMWZSN5rYXRj9CAdheBamobVNubTsi9avR44
  alice         = FXddRd8CdAC8SWKT3Ataasn69R7rbTfQZcKg8ejyrUbF
...
sequenceDiagram
    ...
    participant subscriptions
    participant System
    alice ->> subscriptions: CreateRecurringDelegation (11073cu)
    subscriptions ->> System: CreateAccount
```
...
sequenceDiagram
    ...
    subscriptions ->>+ System: CreateAccount
    System -->>- subscriptions: ok
    subscriptions -->>- alice: ok (11073cu)
```
...
flowchart LR
    ...
    SubAuthority([SubAuthority]):::signer
    9Vct9uPL_iNRB[("9Vct9uPL…iNRB")]:::writable
    2256cKoa_Sja7[("2256cKoa…Sja7")]:::writable
    bob([bob]):::signer
    Token[Token]:::program
    ...
    subscriptions -->|writes| SubAuthority
    subscriptions -->|writes| 9Vct9uPL_iNRB
    subscriptions -->|writes| 2256cKoa_Sja7
    Token -->|writes| 9Vct9uPL_iNRB
    Token -->|writes| 2256cKoa_Sja7
```
...
flowchart LR
    ...
    Token[Token]:::owner
    9Vct9uPL_iNRB[("9Vct9uPL…iNRB")]:::account
    2256cKoa_Sja7[("2256cKoa…Sja7")]:::account
    subscriptions -->|owns| RecurringDelegation
    subscriptions -->|owns| SubAuthority
    Token -->|owns| 9Vct9uPL_iNRB
    Token -->|owns| 2256cKoa_Sja7
```
