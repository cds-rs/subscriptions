## Revoke a subscription with a version mismatch — PASS
...
flowchart LR
    ...
    Eik1rrpG_bdCE[("Eik1rrpG…bdCE")]:::writable
    ...
    subscriptions -->|writes| Eik1rrpG_bdCE
    Token -->|writes| Eik1rrpG_bdCE
```
...
flowchart LR
    ...
    Token[Token]:::owner
    Eik1rrpG_bdCE[("Eik1rrpG…bdCE")]:::account
    System -->|owns| alice
    subscriptions -->|owns| SubAuthority
    Token -->|owns| Eik1rrpG_bdCE
```
...
### Alice tries to revoke a subscription with a zeroed version byte

**RevokeSubscription (version mismatch): structured CPI tree**

```text

── subscriptions::RevokeDelegation ─────────────────────────
Transaction  signers=[alice]
└── subscriptions::RevokeDelegation [1] ✗ 225cu  signer=alice
    └── Error: MigrationRequired (0x86)
Error: custom program error: 0x86
Compute Units (this run): 225
Fee: 0 lamports
Legend (2):
  subscriptions = De1egAFMkMWZSN5rYXRj9CAdheBamobVNubTsi9avR44
  alice         = FXddRd8CdAC8SWKT3Ataasn69R7rbTfQZcKg8ejyrUbF
