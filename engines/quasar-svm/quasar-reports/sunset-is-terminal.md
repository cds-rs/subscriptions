...
sequenceDiagram
    ...
    participant merchant
    participant subscriptions
    merchant ->> subscriptions: UpdatePlan (497cu)
```
...
sequenceDiagram
    ...
    subscriptions -->>- merchant: ok (497cu)
```
...
- [x] the plan is Sunset: `0`

### The merchant tries to revive the plan back to Active

**UpdatePlan (revive after sunset): structured CPI tree**

```text

── subscriptions::UpdatePlan ───────────────────────────────
Transaction  signers=[merchant]
└── subscriptions::UpdatePlan [1] ✗ 251cu  signer=merchant
    └── Error: PlanImmutableAfterSunset (0x201)
Error: custom program error: 0x201
Compute Units (this run): 251
Fee: 0 lamports
Legend (2):
  subscriptions = De1egAFMkMWZSN5rYXRj9CAdheBamobVNubTsi9avR44
  merchant      = J4fPsxKiTTKiXSiN9bgZ5JBrVgTM9c6N8tjhLN8gfdTq
