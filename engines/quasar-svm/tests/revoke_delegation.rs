//! `revoke_delegation` (and `revoke_subscription`), bound to the quasar-svm engine.
//!
...
//! litesvm `test_revoke_delegation_bound.rs`. `bind_scenarios!` emits one
//! `#[test]` per scenario, each calling the generic body with a fresh
//! `make_quasar_backend()`. Every balance assertion is a tolerance inequality
