...
pub fn finding_3_1_1_recurring_pull_before_start_ts<B: TestSVM>(backend: B) {
    ...
    world.md().note(
        "Finding 3.1.1: the delegation's start_ts is a day in the future, yet the pull above lands now. \
         ...
         the fixed code this pull is refused with DelegationNotStarted, and the assertions below confirm the refusal — this regression guards the fix against any future regression.",
    );
