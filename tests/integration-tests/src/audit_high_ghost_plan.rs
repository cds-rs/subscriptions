...
fn finding_3_1_2_ghost_plan_inflated_amount_drains() {
    let mut world = World::new(
        "AUDIT 3.1.2: a ghost plan with an inflated amount drains the subscriber",
    ...
    world.md().note(
        "Finding 3.1.2: Alice consented to 1000/hour, but the merchant deleted and recreated the plan at \
         ...
         and the pull is refused; the checks below then fail — the regression proof.",
    );
    world.md().check("the inflated ghost-plan pull SUCCEEDS (the vulnerability)", true, drained);
    world.md().check("the merchant received 50M", 50_000_000, merchant_got);
    world.md().check("Alice was drained by 50M", alice_before - 50_000_000, alice_after);
}

#[test]
fn finding_3_1_4_ghost_plan_extended_end_ts_siphons() {
    let mut world = World::new(
        "AUDIT 3.1.4: a ghost plan with an extended end_ts siphons past the agreement",
    ...
    world.md().check("the post-original-end pull SUCCEEDS (the vulnerability)", true, siphoned);
    world.md().check("the merchant siphoned 10M past the original end", 10_000_000, merchant_got);
}
